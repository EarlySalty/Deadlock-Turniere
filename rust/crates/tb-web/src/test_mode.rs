...
//! Spieler zu testen: synthetische Test-User (Präfix `test_`) inkl. Zufallsrang
...
//! ([`tb_tournament::finalize_checkin`], [`tb_scheduler::advance_tournament_status`],
//! [`tb_match::MatchManager::apply_group_match_result`] /
//! [`tb_match::MatchManager::apply_bracket_match_result`]) — keine privaten
...
use tb_core::TournamentGameMode;
use tb_match::{ApplyBracketParams, ApplyGroupParams};
use tb_tournament::FinalizeCheckinParams;
...
fn test_mode_enabled() -> bool {
    let raw = tb_config::secrets::get_first_string(&["TURNIER_ENABLE_TEST_MODE"], "1");
    !matches!(
...
// ---------------------------------------------------------------------------
// Mini-PRNG (kein rand-Dependency in tb-web)
// ---------------------------------------------------------------------------
...
fn test_suffix(rng: &mut Rng, existing_ids: &mut HashSet<String>) -> WebResult<String> {
    for _ in 0..1000 {
        let suffix = format!("{:06}", rng.randint(0, 999_999));
        let discord_id = format!("test_{suffix}");
...
fn rank_payload(rng: &mut Rng) -> (String, i64, i64, i64) {
    ...
    let rank_name = tb_steam::rank_name_for_tier(rank_tier)
...
async fn create_test_users_in_db(
    ...
    let existing: Vec<(String,)> =
        sqlx::query_as("SELECT discord_id FROM user_profiles WHERE discord_id LIKE 'test_%'")
            .fetch_all(&mut *conn)
            .await?;
    let mut existing_ids: HashSet<String> = existing
        .into_iter()
        .map(|(d,)| d)
        .filter(|d| !d.is_empty())
        .collect();
    ...
    for _ in 0..count {
        let suffix = test_suffix(rng, &mut existing_ids)?;
        let discord_id = format!("test_{suffix}");
        let display_name = format!("Test User {suffix}");
        let (rank_name, rank_tier, subrank, rank_score) = rank_payload(rng);

        sqlx::query(
            "INSERT INTO user_profiles (discord_id, display_name, updated_at) VALUES (?, ?, ?)",
        )
        .bind(&discord_id)
        .bind(&display_name)
        .bind(&now)
...
where
    E: sqlx::SqliteExecutor<'e>,
{
    let placeholders = vec!["?"; ACTIVE_TOURNAMENT_STATUSES.len()].join(", ");
    let sql = format!(
        "SELECT 1 WHERE EXISTS ( \
            SELECT 1 FROM team_members tm \
            JOIN teams t ON t.id = tm.team_id \
            JOIN tournaments tr ON tr.id = t.tournament_id \
            WHERE tm.discord_id LIKE 'test_%' AND tr.status IN ({ph}) \
         ) OR EXISTS ( \
            SELECT 1 FROM tournament_signups ts \
            JOIN tournaments tr ON tr.id = ts.tournament_id \
            WHERE ts.discord_id LIKE 'test_%' AND tr.status IN ({ph}) \
         ) OR EXISTS ( \
            SELECT 1 FROM tournament_checkins tc \
            JOIN tournaments tr ON tr.id = tc.tournament_id \
            WHERE tc.discord_id LIKE 'test_%' AND tr.status IN ({ph}) \
         ) LIMIT 1",
        ph = placeholders
    );
    let mut query = sqlx::query_scalar::<_, i64>(&sql);
    // Drei Mal die Status-Liste binden (je EXISTS-Block).
    for _ in 0..3 {
        for status in ACTIVE_TOURNAMENT_STATUSES {
            query = query.bind(status);
        }
    }
    let referenced: Option<i64> = query.fetch_optional(executor).await?;
...
/// `bugs_preserved`).
async fn delete_test_users_in_db(conn: &mut sqlx::SqliteConnection) -> WebResult<i64> {
    let deleted: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM user_profiles WHERE discord_id LIKE 'test_%'")
            .fetch_one(&mut *conn)
            .await?;
    for table in [
        "sessions",
        ...
        sqlx::query(&format!(
            "DELETE FROM {table} WHERE discord_id LIKE 'test_%'"
        ))
        .execute(&mut *conn)
        .await?;
    }
...
// ---------------------------------------------------------------------------
// tb-web-lokale Helfer (Audit / Tournament-Load / Tree-Löschung)
// ---------------------------------------------------------------------------
    ...
    E: sqlx::SqliteExecutor<'e>,
{
    let row: Option<TournamentFlags> =
        sqlx::query_as("SELECT status, is_test FROM tournaments WHERE id = ?")
            .bind(tournament_id)
...
/// samt Match-Ergebnissen) und das Turnier selbst. Portiert
/// `_delete_tournament_tree` und `_clear_bracket_tree` 1:1 in der angegebenen
/// Reihenfolge. Executor = Transaktion.
async fn delete_tournament_tree(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    tournament_id: i64,
) -> WebResult<()> {
    let group_ids: Vec<i64> = sqlx::query_scalar("SELECT id FROM groups WHERE tournament_id = ?")
        .bind(tournament_id)
        .fetch_all(&mut **tx)
        .await?;

    if !group_ids.is_empty() {
        let ph = vec!["?"; group_ids.len()].join(", ");
        // match_results der Gruppen-Matches.
        bind_ids(
            sqlx::query(&format!(
                "DELETE FROM match_results WHERE group_match_id IN \
                 (SELECT id FROM group_matches WHERE group_id IN ({ph}))"
            )),
            &group_ids,
        )
        .execute(&mut **tx)
        .await?;
        // checkins der Gruppen-Matches.
        bind_ids(
            sqlx::query(&format!(
        ...
        bind_ids(
            sqlx::query(&format!(
                "DELETE FROM group_matches WHERE group_id IN ({ph})"
            )),
            &group_ids,
        )
        .execute(&mut **tx)
        .await?;
        bind_ids(
            sqlx::query(&format!("DELETE FROM group_teams WHERE group_id IN ({ph})")),
            &group_ids,
        )
        .execute(&mut **tx)
        .await?;
        bind_ids(
            sqlx::query(&format!("DELETE FROM groups WHERE id IN ({ph})")),
            &group_ids,
        )
        .execute(&mut **tx)
        .await?;
    ...
    sqlx::query("DELETE FROM tournament_checkins WHERE tournament_id = ?")
        ...
        .execute(&mut **tx)
        .await?;
    sqlx::query("DELETE FROM tournament_signups WHERE tournament_id = ?")
        .bind(tournament_id)
        .execute(&mut **tx)
        .await?;
    sqlx::query("DELETE FROM teams WHERE tournament_id = ?")
        .bind(tournament_id)
        .execute(&mut **tx)
        .await?;
...
/// Räumt den Bracket-Teilbaum (Matches + Mini-Groups). Portiert
/// `tournament.engine._clear_bracket_tree`.
async fn clear_bracket_tree(
    ...
    let mini_group_ids: Vec<i64> =
        sqlx::query_scalar("SELECT id FROM bracket_mini_groups WHERE tournament_id = ?")
            .bind(tournament_id)
            .fetch_all(&mut **tx)
            .await?;
    if !mini_group_ids.is_empty() {
        let ph = vec!["?"; mini_group_ids.len()].join(", ");
        bind_ids(
            sqlx::query(&format!(
                "DELETE FROM bracket_mini_group_teams WHERE mini_group_id IN ({ph})"
            )),
            &mini_group_ids,
        )
        .execute(&mut **tx)
        ...
        bind_ids(
            sqlx::query(&format!(
                "DELETE FROM bracket_mini_groups WHERE id IN ({ph})"
            )),
            &mini_group_ids,
        )
        .execute(&mut **tx)
...
/// Bindet eine Liste von i64-IDs der Reihe nach an eine dynamische `IN (...)`-Query.
fn bind_ids<'q>(
...
async fn create_test_tournament(
    ...
    let registration_start = now_iso();
    let registration_end = iso_plus_one_hour();
    ...
    // bug-preserved: missing = required - count(aller bestehenden test_-User),
    ...
    .bind(serde_json::to_string(&[1440, 120, 15]).expect("offsets serialisierbar"))
    .fetch_one(&mut *tx)
    .await?;
    ...
    for team_index in 0..body.num_teams {
        ...
        let captain_base = captain
            .display_name
            .clone()
            .unwrap_or_else(|| captain.discord_id.clone());
        ...
        for (member_index, member) in members.iter().enumerate() {
            ...
            let display_name = member
                .display_name
                .clone()
                .unwrap_or_else(|| member.discord_id.clone());
...
async fn advance_test_tournament(
    ...
    if body.advance_to == "group_phase" || body.advance_to == "bracket" {
        ...
        let preview = tb_tournament::finalize_checkin(
            &state.pool,
            tournament_id,
        ...
        tb_tournament::finalize_checkin(
            &state.pool,
            tournament_id,
            FinalizeCheckinParams {
                confirm: true,
    ...
    if body.advance_to == "bracket" {
        ...
        if flags.status == "group_phase" {
            tb_scheduler::advance_tournament_status(
                &state.pool,
                &state.match_manager,
                &state.notifier,
                tournament_id,
...
async fn simulate_test_tournament_round(
    ...
    let finished_ph = vec!["?"; FINISHED_MATCH_STATUSES.len()].join(", ");
    let bracket_rows: Vec<OpenBracketMatch> = bind_status_filter(
        sqlx::query_as::<_, OpenBracketMatch>(&format!(
            "SELECT id, round, team1_id, team2_id \
             FROM bracket_matches \
             WHERE tournament_id = ? \
               AND team1_id IS NOT NULL AND team2_id IS NOT NULL \
               AND status NOT IN ({finished_ph}) \
    ...
    let group_rows: Vec<OpenGroupMatch> = bind_status_filter(
        sqlx::query_as::<_, OpenGroupMatch>(&format!(
            "SELECT gm.id, gm.team1_id, gm.team2_id \
             FROM group_matches gm \
             JOIN groups g ON g.id = gm.group_id \
             WHERE g.tournament_id = ? \
               AND gm.team1_id IS NOT NULL AND gm.team2_id IS NOT NULL \
               AND gm.status NOT IN ({finished_ph}) \
             ORDER BY gm.id"
        ))
        .bind(tournament_id),
    )
...
/// Bindet den `FINISHED_MATCH_STATUSES`-Filter an eine `query_as`.
fn bind_status_filter<'q, T>(
    ...
    for status in FINISHED_MATCH_STATUSES {
        query = query.bind(status);
    }
    query
}
...
async fn wipe_test_data(
    ...
    let tournament_ids: Vec<i64> =
        sqlx::query_scalar("SELECT id FROM tournaments WHERE is_test = 1 ORDER BY id")
            .fetch_all(&mut *tx)
            .await?;
...
/// Aktueller UTC-Zeitstempel als ISO-8601 (`_utc_now().isoformat()`-Äquivalent).
fn now_iso() -> String {
    chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Micros, false)
}

/// Aktueller UTC-Zeitstempel + 1 Stunde als ISO-8601.
fn iso_plus_one_hour() -> String {
