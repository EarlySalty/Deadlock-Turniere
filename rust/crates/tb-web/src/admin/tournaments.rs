...
//! [`tb_scheduler::advance_tournament_status`] mit `source="manual"`.
...
use tb_tournament::{determine_tournament_mode, is_valid_transition, valid_next_statuses};
...
async fn update_tournament(
    ...
    if rebuild_bracket_only {
        tb_tournament::generate_bracket(&state.pool, tournament_id).await?;
        sqlx::query(
...
async fn advance_status_shared(
    ...
    .map_err(|err| match err {
        tb_scheduler::SchedulerError::InvalidTransition(msg) => WebError::bad_request(msg),
        tb_scheduler::SchedulerError::StatusConflict => {
