...
use tb_match::{ApplyBracketParams, MatchError, MatchKind};
...
async fn start_series_game(
    ...
    let game_id = tb_match::series::ensure_game_exists(&state.pool, match_id, game_number).await?;
    let games = tb_match::series::get_series_games(&state.pool, match_id).await?;
...
async fn submit_series_game_result(
    ...
    tb_match::series::ensure_game_exists(&state.pool, match_id, game_number).await?;
    let stats = tb_match::GameStats { duration_s: body.duration_s, ..Default::default() };
    let series_result =
        tb_match::series::record_game_result(&state.pool, match_id, game_number, body.winner_team, &stats)
            .await?;
