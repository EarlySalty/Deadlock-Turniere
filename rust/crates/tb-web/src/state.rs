...
use tb_match::{MatchManager, SteamBridge};
...
pub struct AppState {
    ...
    pub role_sets: tb_auth::RoleSets,
    /// Client für den delegierten Discord-OAuth-Flow.
    pub oauth: tb_auth::OAuthClient,
...
impl AppState {
    ...
    pub async fn build(pool: Pool, config: Arc<Config>) -> Result<Self, BuildError> {
        let role_sets = tb_auth::RoleSets::from_config(&config);
        let oauth = tb_auth::OAuthClient::new(&config);
        ...
        let rank_resolver: Arc<dyn RankResolver> =
            Arc::new(tb_steam::build_resolver(pool.clone(), &config).await?);
...
pub enum BuildError {
    ...
    #[error(transparent)]
    Steam(#[from] tb_steam::SteamError),
}
