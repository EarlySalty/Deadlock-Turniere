//! `tb-match` — der Match-Lebenszyklus des Turnier-Backends.
//!
...
//! [`tb_tournament::advance_bracket_winner`] /
//! [`tb_tournament::complete_mini_group_round_robin`]).
//!
...
//! - [`modes`] — Spielmodi/Hero-Assignment (+ Heldenliste aus [`tb_draft`]).
...
//! Discord ([`tb_discord::DiscordNotifier`]) und die Steam-Bridge
...
/// Steam-Bridge sowie die für den Flow benötigten Config-Werte. tb-web (Welle 5)
/// und tb-scheduler (Welle 4) bauen EINE Instanz und rufen die hier exponierten
/// Methoden.
