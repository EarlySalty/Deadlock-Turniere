//! `tb-app` — Composition-Root und Binary des Turnier-Backends.
//!
...
use tb_discord::{BrokerClient, DiscordNotifier};
use tb_scheduler::{start_scheduler, Scheduler};
use tb_web::{build_router, AppState};
...
async fn main() -> anyhow::Result<()> {
    ...
    let pool = tb_db::connect_str(&config.database_path, DB_MAX_CONNECTIONS).await?;
    tb_db::run_migrations(&pool).await?;
...
fn scheduler_notifier(config: &Config, pool: &tb_db::Pool) -> DiscordNotifier {
