...
//! übersetzte. Hier sind die fachlichen Fälle typisiert, damit tb-web sie gezielt
...
pub enum DraftError {
    ...
    /// `ValueError("{hero} wurde bereits gebannt oder gepickt")`). tb-web → HTTP 400.
    ...
    /// Rust-Port macht den Konflikt explizit. tb-web → HTTP 409.
...
impl From<sqlx::Error> for DraftError {
    fn from(err: sqlx::Error) -> Self {
        DraftError::Db(tb_db::DbError::from(err))
    }
}
