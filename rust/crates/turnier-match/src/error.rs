...
/// `NotFound`/`State`/`Invalid` ersetzt die Python-Subklassen; turnier-api übersetzt
...
impl From<sqlx::Error> for MatchError {
    fn from(err: sqlx::Error) -> Self {
        MatchError::Db(turnier_db::DbError::from(err))
    }
}
...
impl From<sqlx::Error> for SteamTaskError {
    fn from(err: sqlx::Error) -> Self {
        SteamTaskError::Db(turnier_db::DbError::from(err))
    }
}
