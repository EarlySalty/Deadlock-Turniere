...
async fn seed_double_elim_tournament(pool: &Pool) -> i64 {
...
async fn builds_double_elimination_and_grand_final_reset() {
    ...
    let pool = db.pool();
    let tournament_id = seed_double_elim_tournament(pool).await;
    ...
    let type_rows = sqlx::query(
        "SELECT bracket_type, COUNT(*) AS cnt FROM turnier.bracket_matches \
         WHERE tournament_id = $1 GROUP BY bracket_type",
    )
    .bind(tournament_id)
    .fetch_all(pool)
    .await
    ...
    let wr1 = load_matches(pool, tournament_id, "winners", 1).await;
    let wr2 = load_matches(pool, tournament_id, "winners", 2).await;
    let wf = load_matches(pool, tournament_id, "winners", 3).await;
    let lr1 = load_matches(pool, tournament_id, "losers", 1).await;
    let lr2 = load_matches(pool, tournament_id, "losers", 2).await;
    let lr3 = load_matches(pool, tournament_id, "losers", 3).await;
    let lf = load_matches(pool, tournament_id, "losers", 4).await;
    let gf = load_matches(pool, tournament_id, "grand_final", 4).await;
    let gf_reset = load_matches(pool, tournament_id, "grand_final", 5).await;
    ...
    for m in &wr1 {
        advance_bracket_winner(pool, tournament_id, m.0, m.1.unwrap())
            .await
            .expect("advance");
    }
    ...
    assert_eq!(lr1_pairs, vec![(wr1[0].2, wr1[1].2), (wr1[2].2, wr1[3].2)]);
    assert_eq!(wr2_pairs, vec![(wr1[0].1, wr1[1].1), (wr1[2].1, wr1[3].1)]);

    // LR1: jeweils team2 gewinnt.
    advance_bracket_winner(pool, tournament_id, lr1[0].0, lr1[0].2.unwrap())
        .await
        .expect("advance");
    advance_bracket_winner(pool, tournament_id, lr1[1].0, lr1[1].2.unwrap())
        .await
        .expect("advance");
    ...
    assert_eq!(lr2_pairs, vec![(lr1[0].2, wr2[1].2), (lr1[1].2, wr2[0].2)]);
    assert_eq!(wf_pairs, vec![(wr2[0].1, wr2[1].1)]);

    // LR2: jeweils team2 gewinnt.
    advance_bracket_winner(pool, tournament_id, lr2[0].0, lr2[0].2.unwrap())
        .await
        .expect("advance");
    advance_bracket_winner(pool, tournament_id, lr2[1].0, lr2[1].2.unwrap())
        .await
        .expect("advance");

    let lr3 = load_matches(pool, tournament_id, "losers", 3).await;
    let lr3_pairs: Vec<(Option<i64>, Option<i64>)> = lr3.iter().map(|m| (m.1, m.2)).collect();
    assert_eq!(lr3_pairs, vec![(lr2[0].2, lr2[1].2)]);
    advance_bracket_winner(pool, tournament_id, lr3[0].0, lr3[0].1.unwrap())
        .await
        .expect("advance");

    // WF: team1 gewinnt.
    advance_bracket_winner(pool, tournament_id, wf[0].0, wf[0].1.unwrap())
        .await
        .expect("advance");

    let lf = load_matches(pool, tournament_id, "losers", 4).await;
    let lf_pairs: Vec<(Option<i64>, Option<i64>)> = lf.iter().map(|m| (m.1, m.2)).collect();
    assert_eq!(lf_pairs, vec![(lr3[0].1, wf[0].2)]);
    advance_bracket_winner(pool, tournament_id, lf[0].0, lf[0].1.unwrap())
        .await
        .expect("advance");

    let gf = load_matches(pool, tournament_id, "grand_final", 4).await;
    let gf_pairs: Vec<(Option<i64>, Option<i64>)> = gf.iter().map(|m| (m.1, m.2)).collect();
    assert_eq!(gf_pairs, vec![(wf[0].1, lf[0].1)]);
    // GF: team2 (Losers-Finalist) gewinnt -> Reset eager pending.
    advance_bracket_winner(pool, tournament_id, gf[0].0, gf[0].2.unwrap())
        .await
        .expect("advance");

    let gf_reset = load_matches(pool, tournament_id, "grand_final", 5).await;
    assert_eq!(gf_reset[0].5, "pending");
    assert_eq!((gf_reset[0].1, gf_reset[0].2), (wf[0].1, lf[0].1));
}
...
async fn sets_stream_heuristic() {
    ...
    generate_bracket(pool, tournament_id)
        .await
        .expect("generate");

    let rows = sqlx::query(
        "SELECT bracket_type, round, on_stream FROM turnier.bracket_matches WHERE tournament_id = $1",
    )
    .bind(tournament_id)
    .fetch_all(pool)
    .await
    .expect("rows");
    let rows: Vec<(String, i64, bool)> = rows
        .iter()
        .map(|r| {
            (
                r.get::<String, _>("bracket_type"),
                r.get::<i64, _>("round"),
                r.get::<bool, _>("on_stream"),
            )
        })
        .collect();
    ...
    let losers: Vec<&(String, i64, bool)> = rows.iter().filter(|(t, _, _)| t == "losers").collect();
